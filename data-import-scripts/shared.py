...
def upload_data(api: API, dataset: InProgressDataset, dataframe : DataFrame, metadata: dict, description: str, column_descriptions: Optional[Dict[str, str]], changelog: str):
...
def update_dataset(api: API, name: str, dataframe: DataFrame, metadata: dict, description: str, column_descriptions: Optional[Dict[str, str]]):
    ...
    upload_data(api, dataset, dataframe, metadata, description, column_descriptions, "Daily update of data at {}".format(datetime.datetime.now()))
